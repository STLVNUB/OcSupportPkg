  ...
  )
{
  ...
  *PageEntry1G = (UINT64)(UINTN)PageDirectoryEntry | IA32_PG_P | IA32_PG_RW | IA32_PG_US;
  ...
  for (IndexOfPageDirectoryEntries = 0; IndexOfPageDirectoryEntries < 512; IndexOfPageDirectoryEntries++, PageDirectoryEntry++, PhysicalAddress2M += SIZE_2MB) {
    if (((RoStart >= PhysicalAddress2M) && (RoStart < PhysicalAddress2M + SIZE_2MB))
     || ((RoStart + RoSize) > PhysicalAddress2M && ((RoStart + RoSize) <= PhysicalAddress2M + SIZE_2MB))) {
      Split2MPageTo4K (RoStart, RoSize, PhysicalAddress2M, (UINT64 *)PageDirectoryEntry);
    } else {
      //
        ...
        for (IndexOfPageDirectoryEntries = 0; IndexOfPageDirectoryEntries < 512; IndexOfPageDirectoryEntries++, PageDirectoryEntry++, PageAddress += SIZE_2MB) {
          if (((RoStart >= PageAddress) && (RoStart < PageAddress + SIZE_2MB))
           || ((RoStart + RoSize) > PageAddress && ((RoStart + RoSize) <= (PageAddress + SIZE_1GB)))) {
            ...
            //
            Split2MPageTo4K (RoStart, RoSize, PageAddress, (UINT64 *)PageDirectoryEntry);
          } else {
            //
            // Fill in the Page Directory entries
            //
